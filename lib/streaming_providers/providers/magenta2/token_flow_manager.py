...
class TokenFlowManager:
    ...
    def __init__(
        ...
        logger.debug(
            f"TokenFlowManager initialized for {provider_name}"
    ...
    def get_persona_token(self, force_refresh: bool = False) -> PersonaResult:
        ...
        composition_result = PersonaTokenComposer.compose_from_jwt(
            token_result.access_token, MAGENTA2_FALLBACK_ACCOUNT_URI
        )

        if not composition_result:
            logger.debug("=== GET_PERSONA_TOKEN FAILED (composition failed) ===")
            return PersonaResult(success=False, error="Failed to compose persona token")
    ...
    def _get_cached_persona_token(self) -> PersonaResult:
        ...
        try:
            ...
            persona_data = self.session_manager.load_scoped_token(
                self.provider_name, "persona", self.country
            )
            ...
            if (
                persona_data
                and "persona_token" in persona_data
                and "persona_jwt" in persona_data
                and "expires_at" in persona_data
            ):
    ...
    def _cache_persona_composition(self, composition_result) -> None:
        ...
        success = self.session_manager.save_scoped_token(
            self.provider_name, "persona", persona_data, self.country
        )

        if success:
            logger.debug(
                f"✓ Persona token cached until {time.ctime(composition_result.expires_at)}"
            )
        else:
    ...
    def _refresh_yo_digital_if_possible(self) -> TokenFlowResult:
        ...
        try:
            ...
            return TokenFlowResult(
                success=True,
                access_token=new_tokens_dict.get("accessToken")
                or new_tokens_dict.get("access_token"),
                refresh_token=new_tokens_dict.get("refreshToken")
                or new_tokens_dict.get("refresh_token"),
    ...
    def _get_yo_digital_via_taa_exchange(self) -> TokenFlowResult:
        ...
        try:
            ...
            return TokenFlowResult(
                success=False, error=str(e), flow_path="yo_digital_via_taa_exchange"
            )
    ...
    def _get_yo_digital_via_line_auth(self) -> TokenFlowResult:
        ...
        try:
            ...
            return TokenFlowResult(
                success=False, error=str(e), flow_path="yo_digital_via_line_auth"
            )
    ...
    def _get_yo_digital_via_remote_login(self) -> TokenFlowResult:
        ...
        try:
            ...
            return TokenFlowResult(
                success=False, error=str(e), flow_path="yo_digital_via_remote_login"
            )
    ...
    def _is_yo_digital_access_token_valid(token_data: Dict[str, Any]) -> bool:
        ...
        if (
            "access_token_expires_in" not in token_data
            or "access_token_issued_at" not in token_data
        ):
            return False
    ...
    def _is_yo_digital_refresh_token_valid(token_data: Dict[str, Any]) -> bool:
        ...
        if (
            "refresh_token_expires_in" not in token_data
            or "refresh_token_issued_at" not in token_data
        ):
            return False
    ...
    def _save_yo_digital_tokens(self, tokens: Dict[str, Any]) -> None:
        ...
        token_data = {
            ...
            "access_token_expires_in": tokens.get("accessExpiresIn")
            or tokens.get("access_token_expires_in", 86400),
            "access_token_issued_at": current_time,
            "refresh_token": tokens.get("refreshToken") or tokens.get("refresh_token"),
            "refresh_token_expires_in": tokens.get("refreshExpiresIn")
            or tokens.get("refresh_token_expires_in", 86400),
            "refresh_token_issued_at": current_time,
            "token_type": "Bearer",
        }

        success = self.session_manager.save_scoped_token(
            self.provider_name, "yo_digital", token_data, self.country
        )
    ...
    def _save_tvhubs_token(self, token_data: Dict[str, Any]) -> None:
        ...
        self.session_manager.save_scoped_token(
            self.provider_name, "tvhubs", tvhubs_data, self.country
        )
