...
class SettingsManager:
        ...
        if (
            ...
            for provider_name in self.list_registered_providers():
                ...
                if self.provider_supports_countries(provider_name):
                    ...
                    logger.info(
                        f"Syncing multi-country provider {provider_name} for countries: {available_countries}"
                    )
        ...
        try:
            ...
            for provider_name, provider_info in providers_data.items():
                try:
                    ...
                    logger.debug(
                        f"Loaded provider registration: {provider_name} "
    ...
    def get_provider_credentials(
        ...
        if credentials:
            ...
            logger.debug(
                f"SettingsManager: No file credentials found for {provider_name}{country_str}"
            )
    ...
    def save_provider_credentials(
        ...
        if self.kodi_bridge and self.kodi_bridge.is_kodi_environment():
            kodi_success = self.kodi_bridge.write_credentials_to_kodi(
    ...
    def is_provider_registered(self, provider_name: str) -> bool:
        ...
        return (
            provider_name in self._registered_providers
            and self._registered_providers[provider_name].is_active
        )
    ...
    def get_provider_status(
        ...
        status = {
            ...
            "proxy": {"has_proxy": has_proxy, "proxy_enabled": has_proxy},
            "kodi_integration": {
    ...
    def _sync_credentials_from_kodi(
        self, provider_name: str, country: Optional[str] = None
    ) -> bool:
        """
        ...
        if not self.kodi_bridge or not self.kodi_bridge.is_kodi_environment():
            logger.debug(
                f"SettingsManager: Kodi bridge not available for {provider_name}{country_str}"
            )
            return False
        ...
        try:
            ...
            if not kodi_credentials:
                logger.debug(
                    f"SettingsManager: No Kodi credentials found for {provider_name}{country_str}"
                )
                return True  # Not an error

            if not kodi_credentials.validate():
                logger.debug(
                    f"SettingsManager: Invalid Kodi credentials for {provider_name}{country_str}"
                )
                return True  # Not an error
            ...
            logger.info(
                f"SettingsManager: Saving credentials from Kodi for {provider_name}{country_str}"
            )
            success = self.credential_manager.save_credentials(
                provider_name, kodi_credentials, country
            )
            ...
            logger.error(
                f"Error syncing credentials from Kodi for {provider_name}{country_str}: {e}"
            )
            return False
        ...
        try:
            ...
            if country is None and self.provider_supports_countries(provider_name):
                ...
                if reset_credentials:
                    self.credential_manager.delete_credentials(provider_name, country)
                    logger.info(
                        f"Reset credentials for {provider_name}"
                ...
                if reset_proxy:
                    self.proxy_manager.remove_proxy_config(provider_name, country)
                    logger.info(
                        f"Reset proxy config for {provider_name}"
                        + (f" ({country})" if country else "")
                    )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        if self.config_path:
            config_status = {
                ...
                "writable": (
                    os.access(str(self.config_path), os.W_OK)
    ...
    def debug_provider(self, provider_name: str, country: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.kodi_bridge:
            debug_info["kodi"] = {
                ...
                "provider_test": (
                    self.kodi_bridge.test_provider_connectivity(provider_name)
                    if hasattr(self.kodi_bridge, "test_provider_connectivity")
    ...
    def migrate_to_country_structure(self, provider_name: str, default_country: str) -> bool:
        ...
        try:
            logger.info(
                f"Migrating {provider_name} to country structure with default: {default_country}"
            )
            ...
            if old_credentials:
                self.credential_manager.save_credentials(
                    provider_name, old_credentials, default_country
                )
    ...
    def save_provider_credentials_from_api(
        self, provider_name: str, credentials_data: Dict[str, Any]
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if not credentials:
                return (
                    False,
                    "Invalid credentials format. Must provide either (username + password) or (client_id + client_secret)",
                )
    ...
    @staticmethod
    def _parse_credentials_from_dict(
    ...
    def save_provider_proxy_from_api(
        self, provider_name: str, proxy_data: Dict[str, Any]
    ) -> Tuple[bool, str]:
        """
    ...
    def delete_provider_credentials_from_api(self, provider_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if success:
                country_str = f" ({country})" if country else ""
                logger.info(
                    f"Successfully deleted credentials from API for {provider}{country_str}"
                )
    ...
    def delete_provider_proxy_from_api(self, provider_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if success:
                country_str = f" ({country})" if country else ""
                logger.info(
                    f"Successfully deleted proxy config from API for {provider}{country_str}"
                )
